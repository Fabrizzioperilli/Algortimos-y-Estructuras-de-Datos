...
{
  if ((i >= 1 && i <= matrix_.get_m()) && (j >= 1 && j <= matrix_.get_n()) && (matrix_(i, j) != WALL_ID) && (!visited_(i, j)))
    return true;
  else
    return false;
}
...
bool maze_t::solve_(const int i, const int j)
{
  ...
  visited_(i, j) = true; // marcamos la celda como visitada
  ...
  for (int val = N; val <= W; val++)
  {
    int val_i = i + i_d[val];
    int val_j = j + j_d[val];

    if (is_ok_(val_i, val_j))
